...
const makeSut = () => {
  ...
  const tokenGeneratorSpy = makeTokenGenetor()
...
describe('Auth UseCase', () => {
  ...
  test('Should throw if invalid dependencies are provided', async () => {
    ...
    const tokenGenerator = makeTokenGenetor()
    const suts = [].concat(
      new AuthUseCase(),
      new AuthUseCase({}),
      new AuthUseCase({
  ...
  test('Should throw if dependency throw', async () => {
    ...
    const suts = [].concat(
      ...
      new AuthUseCase({
        loadUserByEmailRepository,
        encrypter,
        tokenGeneratorWithError: makeTokenGeneratorWithError()
      })
    )
    for (const sut of suts) {
      const promise = sut.auth('any_email@email.com', 'any_password')
